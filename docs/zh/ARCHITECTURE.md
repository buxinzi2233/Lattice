...
- 所有文件内容先完整写入同目录临时文件、`fsync`，更新时使用原子替换；创建时使用原子链接，避免覆盖并发创建的文件。
- GUI 与 CLI 使用同一个跨进程目录锁；锁等待最多 2 秒并可在退出时取消，每次变更在锁内重新读取最新快照，避免两个入口交叉覆盖。损坏的布局作为 `layout_error` 返回，工具仍可查看和停止，但目录变更会明确失败。
...
应用快照把 `CatalogSnapshot` 与 `StatusSnapshot` 一起返回；状态读取失败会作为 `RuntimeIssue` 返回，并将对应工具标为 `error`，避免把旧状态显示为当前状态。保存草稿、导入规划、自动启动、删除运行保护、分组移动和进程命令均由该门面拥有。
...
真实运行时由单个 `ProcessService` 工作线程调用应用门面，串行处理目录读写、启动分析和进程命令；每个工具的进行中命令不可重复提交，状态轮询合并。`saveToolDraft()` 的返回值表示请求已受理，只有 `toolSaveFinished(id, success)` 表示落盘结果；编辑器等待成功后关闭。文件分析使用 `requestLaunchDraft()` 和 `launchDraftReady`，同步适配器仍可直接调用应用 API。

HTTP/TCP 探测在最多 4 个后台任务中执行，不占用进程锁；结果仅写回相同 `run_id`。日志服务每次读取最多 64 KiB，通过代际编号隔离选择和清空后的旧结果，界面最多保留 2000 行及 256 KiB 字符。硬件采样独立运行。环境准备使用单独任务池和目标目录锁，验证实际解释器的 `sys.prefix`，退出时取消未完成任务。
